...
def Register():
    ...
    #Heading name
    Label(root, text="Flappy Bird", font="ar 15 bold").grid(row=0,column=3)
    ...
    mycur = con.cursor()
    cur.execute("insert into Flappyacc(player_id,player_name,player_email,player_contact,city)values('f001',"+name+","+email+","+contact+","+city+")")
    con.commit()
    con.close()
...
def Ok():
    mysqllob = mysql.connector.connect(host="localhost", user="root", password="", database="")
    mycursor = mysqllob.cursor()
    uname = e1.get()
    password = e2.get()
...
def welcome_page():
    ...
    top.geometry("289x230")
    b1 = Button(top,text = "REGISTER",activeforeground = "white",activebackground = "black",pady=10,command=Register)
    b2 = Button(top,text = "LOGIN",activeforeground = "white",activebackground = "black",pady=10,command=Login)
    b1.pack()
    b2.pack()
    top.mainloop()
